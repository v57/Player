...
let outputCallback: VTDecompressionOutputCallback = {
  refCon, _, status, infoFlags, imageBuffer, pts, _ in
  ...
  c.add(
    FrameCollector.FrameInfo(
      index: index, ptsSeconds: CMTimeGetSeconds(pts), width: CVPixelBufferGetWidth(pb),
      height: CVPixelBufferGetHeight(pb), pixelFormat: CVPixelBufferGetPixelFormatType(pb),
...
print(
  "parsed: \(ps.sps.count) SPS \(ps.sps.map { $0.count })B, \(ps.pps.count) PPS \(ps.pps.map { $0.count })B, NAL length size \(ps.nalLengthSize)"
)
...
let fdStatus = CMVideoFormatDescriptionCreateFromH264ParameterSets(
  ...
  parameterSetSizes: &allSizes, nalUnitHeaderLength: Int32(ps.nalLengthSize),
  formatDescriptionOut: &formatDesc)
guard fdStatus == noErr, let formatDesc else {
  fputs(
    "FATAL: CMVideoFormatDescriptionCreateFromH264ParameterSets failed: OSStatus \(fdStatus)\n",
    stderr)
  exit(1)
}
let dims = CMVideoFormatDescriptionGetDimensions(formatDesc)
print(
  "format description: \(dims.width)x\(dims.height) codec=\(fourccString(CMFormatDescriptionGetMediaSubType(formatDesc)))"
)
...
guard vtStatus == noErr, let session else {
  fputs(
    "FATAL: VTDecompressionSessionCreate failed: OSStatus \(vtStatus) (\(fourccString(UInt32(bitPattern: vtStatus))))\n",
    stderr)
  exit(1)
}
...
for f in collector.frames {
  var line = String(
    format: "frame %3d pts=%8.4fs %4dx%-4d fourcc=%@ planes=%d", f.index, f.ptsSeconds, f.width,
    f.height, fourccString(f.pixelFormat), f.planeCount)
  if f.index == 0 { line += String(format: " bytesPerRow0=%d", f.bytesPerRow0) }
  print(line)
}
if collector.dropped > 0 { print("note: \(collector.dropped) frames dropped by decoder") }
...
check(
  collector.frames.count == fed,
  "all fed packets produced frames (\(collector.frames.count)/\(fed))")
let dimsOK = collector.frames.allSatisfy {
...
var reorderCount = 0
for i in 1..<collector.frames.count
where collector.frames[i].ptsSeconds < collector.frames[i - 1].ptsSeconds { reorderCount += 1 }
let presentationOrder = collector.frames.sorted { $0.ptsSeconds < $1.ptsSeconds }
var ptsIncreasing = true
for i in 1..<presentationOrder.count
where presentationOrder[i].ptsSeconds <= presentationOrder[i - 1].ptsSeconds {
  ptsIncreasing = false
}
check(ptsIncreasing, "pts increasing in presentation order (sorted by pts)")
if reorderCount > 0 {
  print(
    "note: \(reorderCount) frames delivered out of pts order (stream has B-frames; decoder outputs decode order) — client must reorder by pts before display"
  )
}
check(
  collector.decodeErrors.isEmpty && feedErrors.isEmpty,
