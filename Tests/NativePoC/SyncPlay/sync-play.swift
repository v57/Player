...
defer { media_close(da); media_close(dv) }
...
print("audio: \(aframe.nb_samples) samples, \(aframe.channels) ch, \(cstr(media_audio_frame_layout_name(&aframe)))")

// ---------- AVAudioFormat + buffer (5.1(side) -> MPEG_5_1_B) ----------
let layoutTag: AudioChannelLayoutTag = cstr(media_audio_frame_layout_name(&aframe)) == "5.1(side)"
...
let aformat = AVAudioFormat(standardFormatWithSampleRate: Double(sampleRate), channelLayout: channelLayout)
...
var avccSize: Int = 0
guard media_get_track_extradata(dv, Int32(videoStream), &avcc, &avccSize) == MEDIA_RESULT_OK, let avcc, avccSize > 0 else {
...
var off = 6
for _ in 0..<numSPS {
    let len = (Int(avccBytes[off]) << 8) | Int(avccBytes[off + 1]); off += 2
    spsList.append(Array(avccBytes[off..<(off + len)])); off += len
}
let numPPS = Int(avccBytes[off]); off += 1
var ppsList: [[UInt8]] = []
for _ in 0..<numPPS {
    let len = (Int(avccBytes[off]) << 8) | Int(avccBytes[off + 1]); off += 2
    ppsList.append(Array(avccBytes[off..<(off + len)])); off += len
}
var allPtrs: [UnsafePointer<UInt8>] = []
var allSizes: [Int] = []
for s in spsList { s.withUnsafeBufferPointer { allPtrs.append($0.baseAddress!); allSizes.append(s.count) } }
for p in ppsList { p.withUnsafeBufferPointer { allPtrs.append($0.baseAddress!); allSizes.append(p.count) } }
...
let fds = CMVideoFormatDescriptionCreateFromH264ParameterSets(
    ...
    nalUnitHeaderLength: Int32(lengthSizeMinusOne + 1), formatDescriptionOut: &formatDesc)
guard fds == noErr, let formatDesc else { fputs("FATAL: format description\n", stderr); exit(1) }
...
final class FrameBox: @unchecked Sendable {
    ...
    func add(_ pts: Double, _ px: CVPixelBuffer) {
        lock.lock(); frames.append((pts, px)); lock.unlock()
    }
}
let box = FrameBox()
...
var cbRecord = VTDecompressionOutputCallbackRecord(decompressionOutputCallback: callback,
...
let vts = VTDecompressionSessionCreate(allocator: kCFAllocatorDefault, formatDescription: formatDesc,
...
guard vts == noErr, let session else { fputs("FATAL: VT session\n", stderr); exit(1) }
...
while fed < 500 {   // 5 s * 23.976 + margin
    ...
    let bbe = CMBlockBufferCreateWithMemoryBlock(allocator: kCFAllocatorDefault, memoryBlock: pkt.data,
                                                 blockLength: pkt.size, blockAllocator: kCFAllocatorNull,
                                                 customBlockSource: nil, offsetToData: 0, dataLength: pkt.size,
                                                 flags: 0, blockBufferOut: &block)
    guard bbe == noErr, let block else { continue }
    var sbuf: CMSampleBuffer?
    var sampleSize = pkt.size
    let sbs = CMSampleBufferCreate(allocator: kCFAllocatorDefault, dataBuffer: block, dataReady: true,
                                   makeDataReadyCallback: nil, refcon: nil, formatDescription: formatDesc,
                                   sampleCount: 1, sampleTimingEntryCount: 1, sampleTimingArray: &timing,
                                   sampleSizeEntryCount: 1, sampleSizeArray: &sampleSize, sampleBufferOut: &sbuf)
    guard sbs == noErr, let sbuf else { continue }
    // macOS 27 SDK: kVTDecodeFrame_ prefix strips to a bare underscore.
    VTDecompressionSessionDecodeFrame(session, sampleBuffer: sbuf, flags: [._EnableAsynchronousDecompression],
...
print("video: fed \(fed) packets, decoded \(decoded.count) frames, out-of-order pairs in decode order: \(reordered)")
...
print("clock advanced: \(masterClock)s; presented \(presented) frames; max late \(String(format: "%.1f", maxDrift * 1000)) ms; max early \(String(format: "%.1f", maxLatency * 1000)) ms")

check(presented >= expectedFrames - 2, "presented ≈ \(expectedFrames) frames in \(windowSeconds) s (got \(presented))")
check(maxDrift < 0.050, "no frame presented more than 50 ms late (max \(String(format: "%.1f", maxDrift * 1000)) ms)")
check(maxLatency < 0.050, "no frame presented more than 50 ms early (max \(String(format: "%.1f", maxLatency * 1000)) ms)")
