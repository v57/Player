...
for i in 0..<trackCount {
  ...
  print(
    "  [track \(t.id)] type=\(trackTypeName(t)) codec=\(codec) codec_id=\(t.codec_id)"
...
while packetsRead < packetLimit {
  ...
  if r == MEDIA_RESULT_EOF {
    eofHit = true
    break
  }
  if r != MEDIA_RESULT_OK {
    errHit = true
    errDetail = "media_read_packet returned \(r)"
    break
  }
  ...
  print(
    "pkt stream=\(pkt.stream_id) pts=\(pkt.pts) (\(secondsString(pkt.pts, tb))s)"
...
if videoDeltasAll.isEmpty {
  ...
  let dist = videoDeltaMs.sorted { $0.key < $1.key }.map { "\($0.key)ms x\($0.value)" }.joined(
    separator: ", ")
  print("video sanity: \(videoDeltasAll.count) deltas, distribution: \(dist)")
  print(
    "video sanity: nominal 40-43ms: \(nominal)/\(videoDeltasAll.count) (\(String(format: "%.1f", pct))%), negative: \(negative)"
  )
  let pass = pct >= 95.0 && negative == 0
  print(
    "video sanity: \(pass ? "PASS" : "FAIL") — pts cadence consistent with 23.976 fps (41/42 ms at tb 1/1000)"
  )
}
...
while got < 3 {
  ...
  let tb = tbByStream[pkt.stream_id] ?? TB(num: 1, den: 1000)
  afterSeek.append(
    "stream=\(pkt.stream_id) pts=\(pkt.pts) (\(secondsString(pkt.pts, tb))s) key=\(pkt.keyframe)")
  got += 1
}
