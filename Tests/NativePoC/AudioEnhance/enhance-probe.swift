...
AVAudioUnit.instantiate(
    ...
    if let error { print("FATAL: instantiate error: \(error)"); exit(1) }
    matrixAU = au
    sem.signal()
}
sem.wait()
...
let dimErr = AudioUnitGetProperty(matrix.audioUnit, kAudioUnitProperty_MatrixDimensions,
                                  kAudioUnitScope_Global, 0, &dims, &dimSize)
check(dimErr == noErr, "kAudioUnitProperty_MatrixDimensions readable (err=\(dimErr))")
print("  matrix dims: \(dims[0]) in x \(dims[1]) out (AU formats: \(matrix.inputFormat(forBus: 0).channelCount) in / \(matrix.outputFormat(forBus: 0).channelCount) out)")
check(dims[0] == 6, "matrix has 6 inputs")
let ins = Int(dims[0]), outs = Int(dims[1])
...
func buildLevels(centerCoeff cg: Float, flL: Float = 1.0, frR: Float = 1.0, slL: Float = 0.707, srR: Float = 0.707) -> [Float] {
...
func applyLevels(_ levels: [Float]) -> Bool {
    let err = AudioUnitSetProperty(matrix.audioUnit, kAudioUnitProperty_MatrixLevels,
                                   kAudioUnitScope_Global, 0, levels,
...
let getErr = AudioUnitGetProperty(matrix.audioUnit, kAudioUnitProperty_MatrixLevels,
...
func readParam(_ element: AudioUnitElement) -> (Float, OSStatus) {
    var v: Float = -999
    let err = AudioUnitGetParameter(matrix.audioUnit, kMatrixMixerParam_Volume,
                                    kAudioUnitScope_Global, element, &v)
    return (v, err)
}
...
check(abs(c1.left - 0.8 * 0.707) < 0.02 && abs(c1.right - 0.8 * 0.707) < 0.02,
...
check(abs(c2.left - 0.8 * 1.4) < 0.04 && abs(c2.right - 0.8 * 1.4) < 0.04,
...
check((eA2 == noErr && abs(vA2 - 1.4) < 0.01) || (eB2 == noErr && abs(vB2 - 1.4) < 0.01),
