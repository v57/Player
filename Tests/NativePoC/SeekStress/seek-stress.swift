...
check(
  maxPacketsToTarget < 600,
  "target reached within one GOP scan (\(maxPacketsToTarget) packets, ~\(String(format: "%.1f", Double(maxPacketsToTarget) * 41.7 / 1000.0)) s)"
)
check(
  maxGopMs < 15000,
  "keyframe granularity within a plausible GOP (\(String(format: "%.0f", maxGopMs)) ms; long-GOP file, plain seeks are keyframe-coarse by design — exact seek on release covers this)"
)
