...
print("audio track \(track.id): codec=\(cstr(media_track_codec_name(&track))) lang=\(cstr(media_track_language(&track))) channels=\(track.channel_count) rate=\(track.sample_rate)")

// ---------- channel layout (the 5.1 vs 5.1(side) distinction) ----------
var layoutMask: UInt64 = 0
var layoutNameBuf = [CChar](repeating: 0, count: 64)
guard media_get_track_channel_layout(d, Int32(targetAudioStream), &layoutMask, &layoutNameBuf, layoutNameBuf.count) == MEDIA_RESULT_OK else {
...
default:
    layoutTag = kAudioChannelLayoutTag_Unknown
}
print("core audio layout tag: \(layoutTag == kAudioChannelLayoutTag_MPEG_5_1_B ? "MPEG_5_1_B (5.1 side)" : layoutTag == kAudioChannelLayoutTag_MPEG_5_1_A ? "MPEG_5_1_A (5.1 rear)" : "unknown/\(layoutTag)")")
...
print("decoded: \(frame.nb_samples) samples/ch @ \(frame.sample_rate) Hz, \(frame.channels) ch, layout=\"\(cstr(media_audio_frame_layout_name(&frame)))\" fmt=\(cstr(media_audio_frame_sample_fmt(&frame)))")
...
print("AVAudioFormat: rate=\(format.sampleRate) channels=\(format.channelCount) interleaved=\(format.isInterleaved)")
...
for ch in 0..<channels {
    ...
    for i in 0..<Int(frame.nb_samples) {
...
guard let outBuf = AVAudioPCMBuffer(pcmFormat: engine.manualRenderingFormat, frameCapacity: renderFrames) else {
...
print("offline render: status=\(status == .success ? "success" : "failed(\(String(describing: status)))") frames=\(rendered)")
