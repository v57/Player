...
struct MetalRenderTest {
    ...
    static func pixel(_ bytes: [UInt8], width: Int, x: Int, y: Int) -> (r: UInt8, g: UInt8, b: UInt8, a: UInt8) {
        let i = (y * width + x) * 4
    ...
    static func main() {
        ...
        let rect1: DestRect
        do {
            rect1 = try renderer.render(pixelBuffer: pb, into: target1, viewAspect: srcAspect)
        } catch {
        ...
        print(String(format: "dest rect normalized: origin (%.4f, %.4f) size (%.4f, %.4f)  [full frame expected]",
                     rect1.x, rect1.y, rect1.width, rect1.height))
        check(rect1.x == 0 && rect1.y == 0 && rect1.width == 1 && rect1.height == 1,
              "no letterbox when view aspect == source aspect")

        let (egr, egg, egb) = expectedRGB(y: 126, cb: 128, cr: 128)
        let grayExpected = (r: UInt8(round(egr * 255)), g: UInt8(round(egg * 255)), b: UInt8(round(egb * 255)))
        let grayGot = pixel(img1, width: srcW, x: 1000, y: 400)
        print(String(format: "gray pixel  Y=126 Cb=128 Cr=128: expected RGB = (%.4f, %.4f, %.4f) -> bytes (%d,%d,%d), got (%d,%d,%d)",
                     egr, egg, egb, grayExpected.r, grayExpected.g, grayExpected.b,
                     grayGot.r, grayGot.g, grayGot.b))
        check(abs(Int(grayGot.r) - Int(grayExpected.r)) <= tolerance &&
              abs(Int(grayGot.g) - Int(grayExpected.g)) <= tolerance &&
              abs(Int(grayGot.b) - Int(grayExpected.b)) <= tolerance,
              "mid-gray YUV->RGB conversion")
        print("note: Y=126 is 50% luma (Y' = 110/219 = 0.5023); the given matrix outputs 1.1644*0.5023 = 0.585 (149/255) — not 0.5")

        let (err, erg, erb) = expectedRGB(y: 81, cb: 90, cr: 240)
        let redExpected = (r: UInt8(round(err * 255)), g: UInt8(round(erg * 255)), b: UInt8(round(erb * 255)))
        let redGot = pixel(img1, width: srcW, x: 200, y: 100)
        print(String(format: "red pixel   Y=81 Cb=90 Cr=240: expected RGB = (%.4f, %.4f, %.4f) -> bytes (%d,%d,%d), got (%d,%d,%d)",
                     err, erg, erb, redExpected.r, redExpected.g, redExpected.b,
                     redGot.r, redGot.g, redGot.b))
        check(abs(Int(redGot.r) - Int(redExpected.r)) <= tolerance &&
              abs(Int(redGot.g) - Int(redExpected.g)) <= tolerance &&
              abs(Int(redGot.b) - Int(redExpected.b)) <= tolerance,
              "red-ish YUV->RGB conversion")

        // ================= Test 2: letterbox 40:17 into 16:9 =================
        let viewW = 1920, viewH = 1080
        ...
        print(String(format: "source display aspect = %d/%d * sar(1.0) = %.5f  (40:17 = %.5f)",
                     srcW, srcH, srcDisplayAspect, 40.0 / 17.0))
        print(String(format: "dest rect normalized: origin (%.4f, %.4f) size (%.4f, %.4f)",
                     rect2.x, rect2.y, rect2.width, rect2.height))
        let rectPx = (x: rect2.x * Float(viewW), y: rect2.y * Float(viewH),
                      w: rect2.width * Float(viewW), h: rect2.height * Float(viewH))
        print(String(format: "dest rect pixels: x=%.1f y=%.1f w=%.1f h=%.1f",
                     rectPx.x, rectPx.y, rectPx.w, rectPx.h))
        ...
        check(abs(rectPx.y - (Float(viewH) - rectPx.h) / 2) <= 1.5, "letterbox centered vertically (y ≈ 132 px)")
        ...
        print("video pixel (1000,500) got (\(vGray.r),\(vGray.g),\(vGray.b)) expected gray (\(grayExpected.r),\(grayExpected.g),\(grayExpected.b))")
        check(abs(Int(vGray.r) - Int(grayExpected.r)) <= tolerance &&
        ...
        print("red block expected at dest row \(redDestRow) (source row 100); got (\(vRedGot.r),\(vRedGot.g),\(vRedGot.b)) expected (\(redExpected.r),\(redExpected.g),\(redExpected.b))")
        check(abs(Int(vRedGot.r) - Int(redExpected.r)) <= tolerance &&
