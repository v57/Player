...
public final class MetalVideoRenderer {
  ...
  public init(pixelFormat: MTLPixelFormat = .rgba8Unorm) throws {
    ...
    guard let queue = device.makeCommandQueue() else { throw MetalRenderError.commandBufferFailed }
    ...
    guard let vertexFn = library.makeFunction(name: "quadVertex"),
      let fragmentFn = library.makeFunction(name: "yuvFragment")
    else { throw MetalRenderError.libraryCreateFailed("missing quadVertex/yuvFragment functions") }

    let desc = MTLRenderPipelineDescriptor()
    desc.vertexFunction = vertexFn
    desc.fragmentFunction = fragmentFn
    desc.colorAttachments[0].pixelFormat = pixelFormat
    do { self.pipelineState = try device.makeRenderPipelineState(descriptor: desc) } catch {
  ...
  @discardableResult public func render(
    pixelBuffer: CVPixelBuffer, into target: MTLTexture, viewAspect: Float, sar: Float = 1.0
  ) throws -> DestRect {
    guard target.pixelFormat == pixelFormat else { throw MetalRenderError.pixelFormatMismatch }
    ...
    guard let yTex = makeTexture(plane: 0, pixelFormat: .r8Unorm, of: pixelBuffer),
      let uvTex = makeTexture(plane: 1, pixelFormat: .rg8Unorm, of: pixelBuffer)
    else { throw MetalRenderError.textureCreateFailed(kCVReturnError) }
  ...
  private func makeTexture(plane: Int, pixelFormat: MTLPixelFormat, of pixelBuffer: CVPixelBuffer)
    -> MTLTexture?
  {
