...
final class ChannelRoleTests: XCTestCase {
    ...
    func testFivePointOneRear() {
        XCTAssertEqual(
            ChannelRoleMap.roles(forMask: 0x3F),
            [.frontL, .frontR, .center, .lfe, .rearL, .rearR]
        )
    }
    ...
    func testFivePointOneSide() {
        XCTAssertEqual(
            ChannelRoleMap.roles(forMask: 0x60F),
            [.frontL, .frontR, .center, .lfe, .sideL, .sideR]
        )
    }
    ...
    func testStereoMaskReturnsNil() {
        XCTAssertNil(ChannelRoleMap.roles(forMask: 0x3))
    }

    func testZeroMaskReturnsNil() {
        XCTAssertNil(ChannelRoleMap.roles(forMask: 0))
    }

    func testSingleBitMaskReturnsNil() {
        XCTAssertNil(ChannelRoleMap.roles(forMask: 0x4))
    }
    ...
    func testUnknownBitsDegradeToUnknown() {
        XCTAssertEqual(
            ChannelRoleMap.roles(forMask: 0x1C0),
    ...
    func testRoleCountMatchesPopcount() {
        let masks: [UInt64] = [0x3F, 0x60F, 0x63F, 0x1C0, 0x4C0, 0xFFF, 0x8000000000000007]
        for mask in masks {
            XCTAssertEqual(
                ChannelRoleMap.roles(forMask: mask)?.count ?? 0,
