...
final class AudioChainRenderTests: XCTestCase {
    ...
    @discardableResult
    private func renderChunk(engine: AVAudioEngine, into out: AVAudioPCMBuffer) -> Bool {
        guard let scratch = AVAudioPCMBuffer(pcmFormat: stereo, frameCapacity: 1024) else { return false }
    ...
    private func renderOnce(preset: AudioEnhancementPreset, input: AVAudioPCMBuffer) throws -> AVAudioPCMBuffer {
    ...
    func testOriginalIsPassthrough() throws {
        ...
        XCTAssertEqual(rms(out), 0.5 * 0.7071, accuracy: 0.04,
                       "bypassed chain must not alter the signal")
    }
    ...
    func testCompressorTamesLoudInput() throws {
        ...
        print("[Audio] compressor test: quiet \(quietRMS) loud \(loudRMS) ratio \(loudRMS / max(quietRMS, 1e-6)) (input ratio 16)")
        XCTAssertLessThan(loudRMS / max(quietRMS, 1e-6), 10.0, "compression must narrow the quiet/loud gap (input ratio 16x)")
    ...
    func testOutputStaysBelowCeiling() throws {
        ...
        XCTAssertLessThanOrEqual(peak(out), 0.93,
                                 "end-to-end output must stay at/below the -1 dBFS ceiling (0.89 + tolerance)")
    }
    ...
    func testLimiterEngagesInsteadOfHardClipping() throws {
        ...
        let limiterOnly = AudioEnhancementPreset(
            centerGainDB: 0, compressionEnabled: false,
            compressorThresholdDB: base.compressorThresholdDB, compressorHeadroomDB: base.compressorHeadroomDB,
            compressorAttack: base.compressorAttack, compressorRelease: base.compressorRelease,
            compressorMasterGainDB: 0, limiterEnabled: true, limiterPreGainDB: 6,
            limiterAttack: base.limiterAttack, limiterRelease: base.limiterRelease,
    ...
    func testModeSwitchKeepsEngineRunning() throws {
        ...
        XCTAssertEqual(rmsByMode[0], rmsByMode[3], accuracy: 0.06,
    ...
    func testEqBoostsSpeechRangeOnlyInDialogue() throws {
        ...
        XCTAssertGreaterThan(dialogueRatio, balancedRatio * 1.15,
    ...
    func testChainAddsNegligibleLatency() throws {
        ...
        XCTAssertLessThanOrEqual(delta, 1440, "chain adds ≤ 30 ms fixed latency (measured \(delta) frames = \(Double(delta) / rate * 1000) ms)")
        print("[Audio] measured chain latency: \(delta) frames (\(Double(delta) / rate * 1000) ms) — below the 30 ms compensation threshold")
    }
}