...
struct DemuxPacket {
    ...
    var ptsSeconds: Double? {
        pts == Int64.min ? nil : Double(pts) / 1000.0
    }
}
...
struct NativeTrackInfo {
    ...
    init(id: Int, type: Int32, codecID: Int, codecName: String,
         language: String?, title: String?, channelCount: Int, sampleRate: Int,
         timeBase: Rational, isDefault: Bool, isForced: Bool,
...
final class FFmpegDemuxer {
    ...
    func open(_ url: URL) throws {
        close()
        guard let h = url.path.withCString({ media_open($0) }) else {
            throw DemuxError.cannotOpen
        }
        handle = h
    }
    ...
    func track(at index: Int) -> NativeTrackInfo? {
        ...
        var info = NativeTrackInfo(
            id: Int(t.id),
            type: t.type,
            codecID: Int(t.codec_id),
            codecName: String(cString: media_track_codec_name(&t)),
            language: String(cString: media_track_language(&t)),
            title: String(cString: media_track_title(&t)),
            channelCount: Int(t.channel_count),
            sampleRate: Int(t.sample_rate),
            timeBase: Rational(num: t.time_base_num, den: t.time_base_den),
            isDefault: t.is_default != 0,
            isForced: t.is_forced != 0
        )
        if t.type == Int32(MEDIA_TRACK_TYPE_AUDIO.rawValue) {
            var mask: UInt64 = 0
            var name = [CChar](repeating: 0, count: 64)
            if media_get_track_channel_layout(h, Int32(index), &mask, &name, name.count) == MEDIA_RESULT_OK {
    ...
    func seek(to seconds: Double) throws {
        guard let h = handle else { throw DemuxError.readFailed }
        guard media_seek(h, seconds) == MEDIA_RESULT_OK else {
            throw DemuxError.readFailed
        }
    }
    ...
    var chapters: [NativeChapter] {
        ...
        for i in 0..<count {
            ...
            guard media_get_chapter(h, Int32(i), &startMs, &titleBuf, titleBuf.count) == MEDIA_RESULT_OK else { continue }
            let title = String(cString: titleBuf)
            out.append(NativeChapter(title: title.isEmpty ? nil : title,
                                     startTime: Double(startMs) / 1000.0))
        }
        return out
    }
}
