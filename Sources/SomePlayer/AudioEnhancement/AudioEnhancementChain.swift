...
final class AudioEnhancementChain {
  ...
  init?(engine: AVAudioEngine) {
    ...
    guard let dyn = Self.instantiate(kAudioUnitSubType_DynamicsProcessor),
      let lim = Self.instantiate(kAudioUnitSubType_PeakLimiter)
    else { return nil }
    self.dynamics = dyn
    self.limiter = lim
  ...
  func apply(preset: AudioEnhancementPreset) {
    ...
    setParam(
      dynamics, kDynamicsProcessorParam_OverallGain, preset.compressorMasterGainDB, ramp: 0.08)
  ...
  private func setParam(
    _ unit: AVAudioUnit, _ id: AudioUnitParameterID, _ value: Float, ramp: Double
  ) {
    if engine.isRunning {
      unit.auAudioUnit.scheduleParameterBlock(
        AUEventSampleTimeImmediate, AUAudioFrameCount(ramp * 48_000), AUParameterAddress(id), value)
    } else {
  ...
  private func setBypass(_ unit: AVAudioUnit, _ bypass: Bool) {
    ...
    AudioUnitSetProperty(
      unit.audioUnit, kAudioUnitProperty_BypassEffect, kAudioUnitScope_Global, 0, &flag,
