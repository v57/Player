...
/// themselves.
@MainActor final class PlaybackStore {
  ...
  private static let modelContainer: ModelContainer = {
    ...
    let appSupport = URL.applicationSupportDirectory.appendingPathComponent(
      "dev.v57.player", isDirectory: true)
    do {
      try FileManager.default.createDirectory(at: appSupport, withIntermediateDirectories: true)
    } catch { NSLog("[PlaybackStore] cannot create %@: %@", appSupport.path, error as NSError) }
    ...
    if let container = try? ModelContainer(for: schema, configurations: [disk]) { return container }
    ...
    do { return try ModelContainer(for: schema, configurations: [disk]) } catch {
      NSLog("[PlaybackStore] disk container failed, falling back to memory: %@", error as NSError)
  ...
  private static func migrateLegacyDefaultStoreIfNeeded(to storeURL: URL) {
    ...
    guard sqlite3_open_v2(legacy.path, &handle, SQLITE_OPEN_READONLY, nil) == SQLITE_OK, let handle
    else { return }
  ...
  private init() { modelContext = ModelContext(Self.modelContainer) }
  ...
  func fetchRecord(path: String) -> PlaybackRecord? {
    let descriptor = FetchDescriptor<PlaybackRecord>(predicate: #Predicate { $0.filePath == path })
  ...
  func recentVideos(limit: Int = 5) -> [PlaybackRecord] {
    var descriptor = FetchDescriptor<PlaybackRecord>(predicate: #Predicate { $0.position > 0 })
  ...
  func storeTrackPick(kind: String, lang: String?, title: String?, trackID: Int, isOff: Bool) {
    let pick = TrackPick(kind: kind, lang: lang, title: title, trackID: trackID, isOff: isOff)
  ...
  func latestTrackPickRecord(kind: String) -> TrackPick? {
    var descriptor = FetchDescriptor<TrackPick>(predicate: #Predicate { $0.kind == kind })
...
extension PlaybackStore: PlayerPersistence {
  func latestTrackPick(kind: String) -> PlayerTrackPick? {
    latestTrackPickRecord(kind: kind).map {
      PlayerTrackPick(
        kind: $0.kind, lang: $0.lang, title: $0.title, trackID: $0.trackID, isOff: $0.isOff,
        pickedAt: $0.pickedAt)
    }
  }
}
